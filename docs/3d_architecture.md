...
| `sim/js/simulation/smoke3d.js` | 階内拡散、換気・減衰、階段上昇、FDS煙値の優先適用 |
...
cellは少なくとも `walkable`、`wall`、`door`、`stair`、`stairType`、`fire` を持ち、必要に応じて `atrium`、`flammable`、`ventilation`、火災・煙の物理量を持ちます。旧コードとの接続用に `smoke`、`heat`、`co`、`visibility` のaliasも保持できます。
...
FDS連携は `fdsLookup(floorIndex, cx, cy, timeSec)` を境界にします。該当時刻・セルのFDS recordに `temperature` や `heat_flux` があれば、そのフィールドだけfallback値を置き換えます。recordにない物理量はfallbackで補います。したがって優先順位は次です。
...
fallback煙モデルは各階の通行可能セル、火災セル、扉、階段、吹き抜けを対象にします。

- 火災強度に応じた発煙
- 同一階での水平拡散
- 時間減衰と出口・屋外階段による換気
- 階段リンクを通じた下階から上階への移送
- `smokeDensity`、`opticalDensity`、`coPpm`、`visibilityMeters` の導出

`transferLegacySmokeThroughStairs()` は既存の水平拡散後に `floorStates[].smokeMap` のリンク端だけを更新します。既存配列のidentityを維持するため、2D描画や既存参照を壊しません。複数リンクが同じ流出セルを共有する場合は流出総量が元の煙量を超えないよう按分します。

FDS recordに optical density、CO、visibility、smoke density があれば、存在するフィールドをfallbackより優先します。欠けた値はfallbackと相互変換で補完します。これはFDS結果をブラウザ内で再計算するものではなく、外部計算結果を同じfloor/cell/time座標へ重ねる仕組みです。

火災・煙fallbackはいずれも避難挙動を検証するための近似であり、CFD、温度成層、圧力差、扉開閉流を厳密に解くものではありません。
